package io.documentnode.epub4j.search;
...
public class SearchIndex {
  ...
  private static final Logger log = Logger.create(SearchIndex.class);
  ...
  private static final Pattern REMOVE_ACCENT_PATTERN = Pattern
  ...
  private static ResourceSearchIndex createResourceSearchIndex(
      Resource resource) {
    String searchContent = getSearchContent(resource);
    if (StringUtils.isBlank(searchContent)) {
      return null;
    }
    ResourceSearchIndex searchIndex = new ResourceSearchIndex(resource,
        searchContent);
    return searchIndex;
  }
  ...
  private static List<ResourceSearchIndex> createSearchIndex(Book book) {
    ...
    for (Resource resource : book.getContents()) {
      ResourceSearchIndex resourceSearchIndex = createResourceSearchIndex(
          resource);
  ...
  public static String unicodeTrim(String text) {
    ...
    if (leadingWhitespaceCount > 0 || trailingWhitespaceCount > 0) {
      text = text.substring(leadingWhitespaceCount,
          text.length() - trailingWhitespaceCount);
    }
    return text;
  }
  ...
  private static List<SearchResult> doSearch(String searchTerm,
      ResourceSearchIndex resourceSearchIndex) {
    return doSearch(searchTerm, resourceSearchIndex.getContent(),
        resourceSearchIndex.getResource());
  }

  protected static List<SearchResult> doSearch(String searchTerm,
      String content, Resource resource) {
    List<SearchResult> result = new ArrayList<SearchResult>();
    int findPos = content.indexOf(searchTerm);
    while (findPos >= 0) {
      SearchResult searchResult = new SearchResult(findPos, searchTerm,
          resource);
