...
import io.documentnode.epub4j.util.DesktopUtil;
...
public class ContentPane extends JPanel implements NavigationEventListener,
  ...
  private static final Logger log = Logger.create(ContentPane.class);
  ...
  public ContentPane(Navigator navigator) {
        ...
        if (notches < 0) {
          ...
          if (viewPosition.getY() - increment < 0) {
            if (gotoPreviousPage) {
              ...
              ContentPane.this.navigator
                  .gotoPreviousSpineSection(-1, ContentPane.this);
            } else {
    ...
    this.htmlDocumentFactory = new HTMLDocumentFactory(navigator,
   ...
   */
  private static boolean matchesAny(String searchString,
      String... possibleValues) {
    for (int i = 0; i < possibleValues.length; i++) {
      String attributeValue = possibleValues[i];
      if (StringUtils.isNotBlank(attributeValue) && (attributeValue
          .equals(searchString))) {
        return true;
      }
    }
   ...
   */
  private static void scrollToElement(JEditorPane editorPane,
      HTMLDocument.Iterator elementIterator) {
    try {
      Rectangle rectangle = editorPane
          .modelToView(elementIterator.getStartOffset());
      if (rectangle == null) {
        return;
      }
  ...
  private void scrollToNamedAnchor(String fragmentId) {
    ...
    for (HTMLDocument.Iterator iter = doc.getIterator(HTML.Tag.A);
        iter.isValid(); iter.next()) {
      AttributeSet attributes = iter.getAttributes();
      if (matchesAny(fragmentId,
...
//					|| (keyEvent.getKeyCode() == KeyEvent.VK_DOWN)) {
  ...
  private void scrollToCurrentPosition(int sectionPos) {
    ...
    if (sectionPos == 0) {
      ...
      int scrollMax = scrollPane.getVerticalScrollBar().getMaximum();
      scrollPane.getViewport()
          .setViewPosition(new Point(0, scrollMax - viewportHeight));
    }
  }
