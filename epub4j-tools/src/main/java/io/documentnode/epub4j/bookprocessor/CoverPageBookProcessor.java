...
public class CoverPageBookProcessor implements BookProcessor {
  ...
  private static final Logger log = Logger.create(CoverPageBookProcessor.class);
...
//	}
  ...
  private void fixCoverResourceId(Book book, Resource resource,
      String defaultId) {
  ...
  private Resource getFirstImageSource(Resource titlePageResource,
      Resources resources) {
    try {
      Document titlePageDocument = ResourceUtil
          .getAsDocument(titlePageResource);
      NodeList imageElements = titlePageDocument.getElementsByTagName("img");
      for (int i = 0; i < imageElements.getLength(); i++) {
        String relativeImageHref = ((Element) imageElements.item(i))
            .getAttribute("src");
        String absoluteImageHref = calculateAbsoluteImageHref(relativeImageHref,
            titlePageResource.getHref());
  ...
  private String createCoverpageHtml(String title, String imageHref) {
    return "" +
        ...
        + "\" alt=\"" + StringEscapeUtils.escapeHtml(title) + "\"/>\n" +
        "\t\t</div>\n" +
        "\t</body>\n" +
        "</html>\n";
  }
  ...
  private Dimension calculateResizeSize(BufferedImage image) {
    ...
    if (image.getWidth() > image.getHeight()) {
      result = new Dimension(MAX_COVER_IMAGE_SIZE,
          (int) (((double) MAX_COVER_IMAGE_SIZE / (double) image.getWidth())
              * (double) image.getHeight()));
    } else {
      result = new Dimension(
          (int) (((double) MAX_COVER_IMAGE_SIZE / (double) image.getHeight())
              * (double) image.getWidth()), MAX_COVER_IMAGE_SIZE);
    }
    return result;
  }
  ...
  private byte[] createThumbnail(byte[] imageData) throws IOException {
    ...
    BufferedImage thumbnailImage = createResizedCopy(originalImage,
        (int) thumbDimension.getWidth(), (int) thumbDimension.getHeight(),
        false);
